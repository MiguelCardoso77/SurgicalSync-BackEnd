...
public class SpecializationService
{
    ...
    public async Task<List<SpecializationDto>> GetByDesignationAsync(string designation)
    {
        ...
        var filteredList = list.Where(specialization =>
            specialization.Designation.ToString().Contains(designation, StringComparison.OrdinalIgnoreCase)).ToList();
    ...
    {
        if (specializationDto.SpecializationCode.Length <=10 || !System.Text.RegularExpressions.Regex.IsMatch(specializationDto.SpecializationCode, @"^[a-zA-Z0-9\-]+$"))
        {
            throw new ArgumentException("SpecializationCode must be less than 10 characters long and contain only letters, numbers, and dashes.");
        }
