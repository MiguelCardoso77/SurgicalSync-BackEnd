...
The client has outlined that the Admin Role should possess the functionality to remove existing operation types.

Admins can mark operation types as inactive, rather than deleting them, to preserve historical records. Inactive operation types are no longer available for future scheduling but remain in historical data.

A confirmation prompt must be shown before deactivating an operation type.
